...
We begin by computing the **sample mean return vector** of $n$ assets,
$\hat{\boldsymbol\mu}\in\mathbb R^n$,
also known as the **expected return vector**, as the column‑wise average of each asset’s [log‑returns](log_returns.md#log‑returns) over the $T-1$ observations. As detailed in [log‑returns](log_returns.md#log‑returns), “return” refers to log‑returns; see [Osborne’s work](normality.md#osborne) for why log‑returns are preferred.
...
= \frac{1}{T-1}
\sum_{t=1}^{T-1}
\bigl(\mathbf r_t - \hat{\boldsymbol\mu}\bigr)
\bigl(\mathbf r_t - \hat{\boldsymbol\mu}\bigr)^\top,
$$
...
To calculate the sample covariance matrix we build the [return matrix](expected_returns.md#expected-returns) by stacking log‑returns into an $(T-1)\times n$ matrix
...
contains the log‑returns of asset $i$:

$$
r_t^{(i)} = \ln\bigl(\tfrac{p_{t+1}^{(i)}}{p_t^{(i)}}\bigr).
$$

Then we center the return matrix by subtracting each column’s mean from its entries, i.e.\ 
$$
\tilde R = R - \mathbf1\,\hat{\boldsymbol\mu}^\top,
\quad
\hat{\boldsymbol\mu} = \frac{1}{T-1}\,R^\top\mathbf1.
$$
...
= \frac{1}{T-1} \sum_{t=1}^{T-1} (\mathbf r_t - \hat\mu)(\mathbf r_t - \hat\mu)^\top.
$$

**Note:** Because $\hat{\boldsymbol\mu}$ itself is a function of log‑returns, the sample covariance ultimately builds on log‑returns. Ensuring log‑returns are well‑justified (e.g., by the normality assumption in [Osborne's work](normality.md#osborne)) is therefore crucial, as it underpins every step of this construction.

---

**Numerical Example: Sample Covariance Matrix**

Using our nine days of log‑returns for six assets (AAPL, AMZN, GOOG, MSFT, TQQQ, TSLA) from [the numerical example](expected_returns.md#expected-returns), we obtain:
...
Because both frameworks rely on the same $\hat\Sigma$ built from log‑returns and the sample mean, the [**justification of log‑returns’ properties**](normality.md#osborne) (e.g., approximate normality) propagates through to your portfolio construction rules.
